// Initialize Question74
export function initQ73(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-73");
  const submitEl = document.getElementById("submit-73");
  const expEl = document.getElementById("exp-73");
  const verdictEl = document.getElementById("verdict-73");
  const card = document.getElementById("card-73");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[73]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[73].includes(idx);
      ...
      if (isAlreadySelected && isCorrectAnswer) {
        ...
        indicator.innerHTML = "";
        state.answers[73] = state.answers[73].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[73].includes(idx)) {
        state.answers[73].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[73].every(correctIdx => 
        state.answers[73].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[73].length === CORRECT[73].length) {
        // All correct answers found!
        revealAnswer(73, true);
      }
    });
  });
