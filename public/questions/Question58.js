// Initialize Question58
export function initQ57({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-57");
  const submitEl = document.getElementById("submit-57");
  const expEl = document.getElementById("exp-57");
  const verdictEl = document.getElementById("verdict-57");
  const card = document.getElementById("card-57");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[57]) return;
      ...
      state.answers[57] = parseInt(btn.dataset.idx);
