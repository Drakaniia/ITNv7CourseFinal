// Initialize Question47
export function initQ46({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-46");
  const submitEl = document.getElementById("submit-46");
  const expEl = document.getElementById("exp-46");
  const verdictEl = document.getElementById("verdict-46");
  const card = document.getElementById("card-46");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[46]) return;
      ...
      state.answers[46] = parseInt(btn.dataset.idx);
