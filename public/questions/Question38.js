// Initialize Question38
export function initQ37({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-37");
  const submitEl = document.getElementById("submit-37");
  const expEl = document.getElementById("exp-37");
  const verdictEl = document.getElementById("verdict-37");
  const card = document.getElementById("card-37");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[37]) return;
      ...
      state.answers[37] = parseInt(btn.dataset.idx);
