// Initialize Question43
export function initQ42({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-42");
  const submitEl = document.getElementById("submit-42");
  const expEl = document.getElementById("exp-42");
  const verdictEl = document.getElementById("verdict-42");
  const card = document.getElementById("card-42");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[42]) return;
      ...
      state.answers[42] = parseInt(btn.dataset.idx);
