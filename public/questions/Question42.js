// Initialize Question42
export function initQ41({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-41");
  const submitEl = document.getElementById("submit-41");
  const expEl = document.getElementById("exp-41");
  const verdictEl = document.getElementById("verdict-41");
  const card = document.getElementById("card-41");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[41]) return;
      ...
      state.answers[41] = parseInt(btn.dataset.idx);
