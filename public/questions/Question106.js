// Initialize Question106
export function initQ105({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-105");
  const submitEl = document.getElementById("submit-105");
  const expEl = document.getElementById("exp-105");
  const verdictEl = document.getElementById("verdict-105");
  const card = document.getElementById("card-105");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[105]) return;
      ...
      state.answers[105] = parseInt(btn.dataset.idx);

      // Immediately reveal answer (direct reveal)
      revealAnswer(105, btn);
    });
  });
