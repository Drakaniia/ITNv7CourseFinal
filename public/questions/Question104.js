// Initialize Question104
export function initQ103({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-103");
  const submitEl = document.getElementById("submit-103");
  const expEl = document.getElementById("exp-103");
  const verdictEl = document.getElementById("verdict-103");
  const card = document.getElementById("card-103");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[103]) return;
      ...
      state.answers[103] = parseInt(btn.dataset.idx);

      // Immediately reveal answer (direct reveal)
      revealAnswer(103, btn);
    });
  });
