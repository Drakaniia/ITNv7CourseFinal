// Initialize Question85
export function initQ84({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-84");
  const submitEl = document.getElementById("submit-84");
  const expEl = document.getElementById("exp-84");
  const verdictEl = document.getElementById("verdict-84");
  const card = document.getElementById("card-84");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[84]) return;
      ...
      state.answers[84] = parseInt(btn.dataset.idx);
