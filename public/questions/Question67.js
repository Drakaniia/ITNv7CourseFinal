// Initialize Question67
export function initQ66({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-66");
  const submitEl = document.getElementById("submit-66");
  const expEl = document.getElementById("exp-66");
  const verdictEl = document.getElementById("verdict-66");
  const card = document.getElementById("card-66");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[66]) return;
      ...
      state.answers[66] = parseInt(btn.dataset.idx);
