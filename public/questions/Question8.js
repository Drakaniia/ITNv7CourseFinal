// Initialize Question8
export function initQ7({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-7");
  const submitEl = document.getElementById("submit-7");
  const expEl = document.getElementById("exp-7");
  const verdictEl = document.getElementById("verdict-7");
  const card = document.getElementById("card-7");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[7]) return;
      ...
      state.answers[7] = parseInt(btn.dataset.idx);
