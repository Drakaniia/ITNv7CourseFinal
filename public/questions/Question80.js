// Initialize Question80
export function initQ79({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-79");
  const submitEl = document.getElementById("submit-79");
  const expEl = document.getElementById("exp-79");
  const verdictEl = document.getElementById("verdict-79");
  const card = document.getElementById("card-79");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[79]) return;
      ...
      state.answers[79] = parseInt(btn.dataset.idx);
