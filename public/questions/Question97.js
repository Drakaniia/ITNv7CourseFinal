// Initialize Question97
export function initQ96(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-96");
  const submitEl = document.getElementById("submit-96");
  const expEl = document.getElementById("exp-96");
  const verdictEl = document.getElementById("verdict-96");
  const card = document.getElementById("card-96");
  const btns = optsEl.querySelectorAll(".option-btn");
  const pendingCountEl = document.getElementById("pendingCount-96");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (isSelected) {
        ...
        if (!state.answers[96].includes(idx)) {
          state.answers[96].push(idx);
        }
      } else {
        indicator.innerHTML = "";
        state.answers[96] = state.answers[96].filter((i) => i !== idx);
      }

      // Update pending count
      const pendingCount = state.answers[96].length;
