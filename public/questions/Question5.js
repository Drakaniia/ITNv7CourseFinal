// Initialize Question5
export function initQ4({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-4");
  const submitEl = document.getElementById("submit-4");
  const expEl = document.getElementById("exp-4");
  const verdictEl = document.getElementById("verdict-4");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      state.answers[4] = parseInt(btn.dataset.idx);
