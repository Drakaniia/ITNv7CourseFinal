// Initialize Question108
export function initQ107({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-107");
  const submitEl = document.getElementById("submit-107");
  const expEl = document.getElementById("exp-107");
  const verdictEl = document.getElementById("verdict-107");
  const card = document.getElementById("card-107");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[107]) return;
      ...
      state.answers[107] = parseInt(btn.dataset.idx);

      // Immediately reveal answer (direct reveal)
      revealAnswer(107, btn);
    });
  });
