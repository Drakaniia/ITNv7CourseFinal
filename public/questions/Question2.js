// Initialize Question2
export function initQ1(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[1].includes(idx);
      ...
      if (isAlreadySelected && isCorrectAnswer) {
        ...
        indicator.innerHTML = "";
        state.answers[1] = state.answers[1].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[1].includes(idx)) {
        state.answers[1].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[1].every(correctIdx => 
        state.answers[1].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[1].length === CORRECT[1].length) {
