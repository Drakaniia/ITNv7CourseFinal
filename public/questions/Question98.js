// Initialize Question98
export function initQ97({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-97");
  const submitEl = document.getElementById("submit-97");
  const expEl = document.getElementById("exp-97");
  const verdictEl = document.getElementById("verdict-97");
  const card = document.getElementById("card-97");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[97]) return;
      ...
      state.answers[97] = parseInt(btn.dataset.idx);
