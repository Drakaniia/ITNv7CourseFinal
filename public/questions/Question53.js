// Initialize Question53
export function initQ52({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-52");
  const submitEl = document.getElementById("submit-52");
  const expEl = document.getElementById("exp-52");
  const verdictEl = document.getElementById("verdict-52");
  const card = document.getElementById("card-52");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[52]) return;
      ...
      state.answers[52] = parseInt(btn.dataset.idx);
