// Initialize Question45
export function initQ44(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-44");
  const submitEl = document.getElementById("submit-44");
  const expEl = document.getElementById("exp-44");
  const verdictEl = document.getElementById("verdict-44");
  const card = document.getElementById("card-44");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[44]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[44].includes(idx);
      ...
      if (isAlreadySelected && isCorrectAnswer) {
        ...
        indicator.innerHTML = "";
        state.answers[44] = state.answers[44].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[44].includes(idx)) {
        state.answers[44].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[44].every(correctIdx => 
        state.answers[44].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[44].length === CORRECT[44].length) {
        // All correct answers found!
        revealAnswer(44, true);
      }
    });
  });
