// Initialize Question82
export function initQ81({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-81");
  const submitEl = document.getElementById("submit-81");
  const expEl = document.getElementById("exp-81");
  const verdictEl = document.getElementById("verdict-81");
  const card = document.getElementById("card-81");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[81]) return;
      ...
      state.answers[81] = parseInt(btn.dataset.idx);
