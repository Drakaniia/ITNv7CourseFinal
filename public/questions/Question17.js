// Initialize Question17
export function initQ16({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-16");
  const submitEl = document.getElementById("submit-16");
  const expEl = document.getElementById("exp-16");
  const verdictEl = document.getElementById("verdict-16");
  const card = document.getElementById("card-16");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[16]) return;
      ...
      state.answers[16] = parseInt(btn.dataset.idx);
