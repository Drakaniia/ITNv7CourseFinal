// Initialize Question102
export function initQ101(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-101");
  const submitEl = document.getElementById("submit-101");
  ...
  const pendingCountEl = document.getElementById("pendingCount-101");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (isSelected) {
        ...
        if (!state.answers[101].includes(idx)) {
          state.answers[101].push(idx);
        }
      } else {
        indicator.innerHTML = "";
        state.answers[101] = state.answers[101].filter((i) => i !== idx);
      }

      // Update pending count
      const pendingCount = state.answers[101].length;
