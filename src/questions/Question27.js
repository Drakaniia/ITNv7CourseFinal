// Initialize Question27
export function initQ26(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-26");
  const submitEl = document.getElementById("submit-26");
  const expEl = document.getElementById("exp-26");
  const verdictEl = document.getElementById("verdict-26");
  const card = document.getElementById("card-26");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[26]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[26].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[26] = state.answers[26].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[26].includes(idx)) {
        state.answers[26].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[26].every(correctIdx => 
        state.answers[26].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[26].length === CORRECT[26].length) {
        // All correct answers found!
        revealAnswer(26, true);
      }
    });
  });
