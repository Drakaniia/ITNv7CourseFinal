...
export function initQ23(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[23].includes(idx);
      ...
      const allCorrectSelected = CORRECT[23].every(correctIdx => 
        state.answers[23].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[23].length === CORRECT[23].length) {
        // All correct answers found!
        revealAnswer(23, true);
      }
    });
  });
