// Initialize Question103
export function initQ102(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-102");
  const submitEl = document.getElementById("submit-102");
  const expEl = document.getElementById("exp-102");
  const verdictEl = document.getElementById("verdict-102");
  const card = document.getElementById("card-102");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[102]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[102].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[102] = state.answers[102].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[102].includes(idx)) {
        state.answers[102].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[102].every(correctIdx => 
        state.answers[102].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[102].length === CORRECT[102].length) {
        // All correct answers found!
        revealAnswer(102, true);
      }
    });
  });
