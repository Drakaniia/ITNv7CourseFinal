...
export function initQ2(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[2].includes(idx);
      ...
      const allCorrectSelected = CORRECT[2].every(correctIdx => 
        state.answers[2].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[2].length === CORRECT[2].length) {
