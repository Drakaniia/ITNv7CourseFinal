...
export function initQ17(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[17].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[17] = state.answers[17].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[17].includes(idx)) {
        state.answers[17].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[17].every(correctIdx => 
        state.answers[17].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[17].length === CORRECT[17].length) {
        // All correct answers found!
        revealAnswer(17, true);
      }
    });
  });
