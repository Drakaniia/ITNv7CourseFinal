// Initialize Question79
export function initQ78({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-78");
  const submitEl = document.getElementById("submit-78");
  const expEl = document.getElementById("exp-78");
  const verdictEl = document.getElementById("verdict-78");
  const card = document.getElementById("card-78");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[78]) return;
      ...
      state.answers[78] = parseInt(btn.dataset.idx);
