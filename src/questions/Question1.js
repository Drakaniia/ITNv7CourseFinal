...
export function initQ0(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[0].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[0] = state.answers[0].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[0].includes(idx)) {
        state.answers[0].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[0].every(correctIdx => 
        state.answers[0].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[0].length === CORRECT[0].length) {
