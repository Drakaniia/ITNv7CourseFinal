// Initialize Question61
export function initQ60(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-60");
  const submitEl = document.getElementById("submit-60");
  const expEl = document.getElementById("exp-60");
  const verdictEl = document.getElementById("verdict-60");
  const card = document.getElementById("card-60");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[60]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[60].includes(idx);
      ...
      if (isAlreadySelected && isCorrectAnswer) {
        ...
        indicator.innerHTML = "";
        state.answers[60] = state.answers[60].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[60].includes(idx)) {
        state.answers[60].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[60].every(correctIdx => 
        state.answers[60].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[60].length === CORRECT[60].length) {
        // All correct answers found!
        revealAnswer(60, true);
      }
    });
  });
