// Initialize Question20
export function initQ19(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-19");
  const submitEl = document.getElementById("submit-19");
  const expEl = document.getElementById("exp-19");
  const verdictEl = document.getElementById("verdict-19");
  const card = document.getElementById("card-19");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[19]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[19].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[19] = state.answers[19].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[19].includes(idx)) {
        state.answers[19].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[19].every(correctIdx => 
        state.answers[19].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[19].length === CORRECT[19].length) {
        // All correct answers found!
        revealAnswer(19, true);
      }
    });
  });
