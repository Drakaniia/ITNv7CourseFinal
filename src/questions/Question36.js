// Initialize Question36
export function initQ35({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-35");
  const submitEl = document.getElementById("submit-35");
  const expEl = document.getElementById("exp-35");
  const verdictEl = document.getElementById("verdict-35");
  const card = document.getElementById("card-35");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[35]) return;
      ...
      state.answers[35] = parseInt(btn.dataset.idx);
