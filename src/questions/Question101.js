// Initialize Question101
export function initQ100(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-100");
  const submitEl = document.getElementById("submit-100");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[100]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[100].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[100] = state.answers[100].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[100].includes(idx)) {
        state.answers[100].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[100].every(correctIdx => 
        state.answers[100].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[100].length === CORRECT[100].length) {
        // All correct answers found!
        revealAnswer(100, true);
      }
    });
  });
