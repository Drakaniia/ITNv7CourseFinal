// Initialize Question109
export function initQ108(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-108");
  const submitEl = document.getElementById("submit-108");
  const expEl = document.getElementById("exp-108");
  const verdictEl = document.getElementById("verdict-108");
  const card = document.getElementById("card-108");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[108]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[108].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[108] = state.answers[108].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[108].includes(idx)) {
        state.answers[108].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[108].every(correctIdx => 
        state.answers[108].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[108].length === CORRECT[108].length) {
        // All correct answers found!
        revealAnswer(108, true);
      }
    });
  });
