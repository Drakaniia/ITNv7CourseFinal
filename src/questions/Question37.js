// Initialize Question37
export function initQ36({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-36");
  const submitEl = document.getElementById("submit-36");
  const expEl = document.getElementById("exp-36");
  const verdictEl = document.getElementById("verdict-36");
  const card = document.getElementById("card-36");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[36]) return;
      ...
      state.answers[36] = parseInt(btn.dataset.idx);
