// Initialize Question41
export function initQ40({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-40");
  const submitEl = document.getElementById("submit-40");
  const expEl = document.getElementById("exp-40");
  const verdictEl = document.getElementById("verdict-40");
  const card = document.getElementById("card-40");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[40]) return;
      ...
      state.answers[40] = parseInt(btn.dataset.idx);
