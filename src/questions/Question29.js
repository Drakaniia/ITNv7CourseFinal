// Initialize Question29
export function initQ28(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-28");
  const submitEl = document.getElementById("submit-28");
  const expEl = document.getElementById("exp-28");
  const verdictEl = document.getElementById("verdict-28");
  const card = document.getElementById("card-28");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[28]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[28].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[28] = state.answers[28].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[28].includes(idx)) {
        state.answers[28].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[28].every(correctIdx => 
        state.answers[28].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[28].length === CORRECT[28].length) {
        // All correct answers found!
        revealAnswer(28, true);
      }
    });
  });
