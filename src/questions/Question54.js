// Initialize Question54
export function initQ53({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-53");
  const submitEl = document.getElementById("submit-53");
  const expEl = document.getElementById("exp-53");
  const verdictEl = document.getElementById("verdict-53");
  const card = document.getElementById("card-53");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[53]) return;
      ...
      state.answers[53] = parseInt(btn.dataset.idx);
