// Initialize Question50
export function initQ49(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-49");
  const submitEl = document.getElementById("submit-49");
  const expEl = document.getElementById("exp-49");
  const verdictEl = document.getElementById("verdict-49");
  const card = document.getElementById("card-49");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[49]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[49].includes(idx);
      ...
      if (isAlreadySelected && isCorrectAnswer) {
        ...
        indicator.innerHTML = "";
        state.answers[49] = state.answers[49].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[49].includes(idx)) {
        state.answers[49].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[49].every(correctIdx => 
        state.answers[49].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[49].length === CORRECT[49].length) {
        // All correct answers found!
        revealAnswer(49, true);
      }
    });
  });
