// Initialize Question94
export function initQ93({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-93");
  const submitEl = document.getElementById("submit-93");
  const expEl = document.getElementById("exp-93");
  const verdictEl = document.getElementById("verdict-93");
  const card = document.getElementById("card-93");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[93]) return;
      ...
      state.answers[93] = parseInt(btn.dataset.idx);
