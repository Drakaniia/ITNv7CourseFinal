// Initialize Question48
export function initQ47({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-47");
  const submitEl = document.getElementById("submit-47");
  const expEl = document.getElementById("exp-47");
  const verdictEl = document.getElementById("verdict-47");
  const card = document.getElementById("card-47");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[47]) return;
      ...
      state.answers[47] = parseInt(btn.dataset.idx);
