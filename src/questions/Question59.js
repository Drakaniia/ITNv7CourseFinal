// Initialize Question59
export function initQ58(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-58");
  const submitEl = document.getElementById("submit-58");
  const expEl = document.getElementById("exp-58");
  const verdictEl = document.getElementById("verdict-58");
  const card = document.getElementById("card-58");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[58]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[58].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[58] = state.answers[58].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[58].includes(idx)) {
        state.answers[58].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[58].every(correctIdx => 
        state.answers[58].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[58].length === CORRECT[58].length) {
        // All correct answers found!
        revealAnswer(58, true);
      }
    });
  });
