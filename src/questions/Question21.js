// Initialize Question21
export function initQ20(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-20");
  const submitEl = document.getElementById("submit-20");
  const expEl = document.getElementById("exp-20");
  const verdictEl = document.getElementById("verdict-20");
  const card = document.getElementById("card-20");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[20]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[20].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[20] = state.answers[20].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[20].includes(idx)) {
        state.answers[20].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[20].every(correctIdx => 
        state.answers[20].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[20].length === CORRECT[20].length) {
        // All correct answers found!
        revealAnswer(20, true);
      }
    });
  });
