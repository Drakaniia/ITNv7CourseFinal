...
export function initQ94(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[94].includes(idx);
      ...
      if (isAlreadySelected && isCorrectAnswer) {
        ...
        indicator.innerHTML = "";
        state.answers[94] = state.answers[94].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[94].includes(idx)) {
        state.answers[94].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[94].every(correctIdx => 
        state.answers[94].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[94].length === CORRECT[94].length) {
        // All correct answers found!
        revealAnswer(94, true);
      }
    });
  });
