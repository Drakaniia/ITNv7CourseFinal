...
export function initQ10(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[10].includes(idx);
      ...
      if (!state.answers[10].includes(idx)) {
        state.answers[10].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[10].every(correctIdx => 
        state.answers[10].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[10].length === CORRECT[10].length) {
        // All correct answers found!
        revealAnswer(10, true);
      }
    });
  });
