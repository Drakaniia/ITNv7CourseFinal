// Initialize Question87
export function initQ86({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-86");
  const submitEl = document.getElementById("submit-86");
  const expEl = document.getElementById("exp-86");
  const verdictEl = document.getElementById("verdict-86");
  const card = document.getElementById("card-86");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[86]) return;
      ...
      state.answers[86] = parseInt(btn.dataset.idx);
