...
export function initQ9(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[9].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[9] = state.answers[9].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[9].includes(idx)) {
        state.answers[9].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[9].every(correctIdx => 
        state.answers[9].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[9].length === CORRECT[9].length) {
