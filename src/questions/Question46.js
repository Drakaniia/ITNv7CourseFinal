// Initialize Question46
export function initQ45(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-45");
  const submitEl = document.getElementById("submit-45");
  const expEl = document.getElementById("exp-45");
  const verdictEl = document.getElementById("verdict-45");
  const card = document.getElementById("card-45");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[45]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[45].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[45] = state.answers[45].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[45].includes(idx)) {
        state.answers[45].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[45].every(correctIdx => 
        state.answers[45].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[45].length === CORRECT[45].length) {
        // All correct answers found!
        revealAnswer(45, true);
      }
    });
  });
