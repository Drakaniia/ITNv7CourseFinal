// Initialize Question39
export function initQ38({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-38");
  const submitEl = document.getElementById("submit-38");
  const expEl = document.getElementById("exp-38");
  const verdictEl = document.getElementById("verdict-38");
  const card = document.getElementById("card-38");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[38]) return;
      ...
      state.answers[38] = parseInt(btn.dataset.idx);
