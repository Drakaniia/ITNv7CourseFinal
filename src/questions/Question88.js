...
export function initQ87(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[87].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[87] = state.answers[87].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[87].includes(idx)) {
        state.answers[87].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[87].every(correctIdx => 
        state.answers[87].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[87].length === CORRECT[87].length) {
        // All correct answers found!
        revealAnswer(87, true);
      }
    });
  });
