...
export function initQ56(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[56].includes(idx);
      ...
      if (isAlreadySelected && isCorrectAnswer) {
        ...
        indicator.innerHTML = "";
        state.answers[56] = state.answers[56].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[56].includes(idx)) {
        state.answers[56].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[56].every(correctIdx => 
        state.answers[56].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[56].length === CORRECT[56].length) {
        // All correct answers found!
        revealAnswer(56, true);
      }
    });
  });
