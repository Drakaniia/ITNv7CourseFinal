...
export function initQ43(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[43].includes(idx);
      ...
      const allCorrectSelected = CORRECT[43].every(correctIdx => 
        state.answers[43].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[43].length === CORRECT[43].length) {
        // All correct answers found!
        revealAnswer(43, true);
      }
    });
  });
