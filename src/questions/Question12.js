// Initialize Question12
export function initQ11(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-11");
  const submitEl = document.getElementById("submit-11");
  const expEl = document.getElementById("exp-11");
  const verdictEl = document.getElementById("verdict-11");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[11]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[11].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[11] = state.answers[11].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[11].includes(idx)) {
        state.answers[11].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[11].every(correctIdx => 
        state.answers[11].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[11].length === CORRECT[11].length) {
        // All correct answers found!
        revealAnswer(11, true);
      }
    });
  });
