// Initialize Question100
export function initQ99({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-99");
  const submitEl = document.getElementById("submit-99");
  const expEl = document.getElementById("exp-99");
  const verdictEl = document.getElementById("verdict-99");
  const card = document.getElementById("card-99");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[99]) return;
      ...
      state.answers[99] = parseInt(btn.dataset.idx);
