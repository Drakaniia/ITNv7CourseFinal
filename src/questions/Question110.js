...
export function initQ109(ctx) {
  ...
  const pendingCountEl = document.getElementById("pendingCount-109");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (isSelected) {
        ...
        if (!state.answers[109].includes(idx)) {
          state.answers[109].push(idx);
        }
      } else {
        indicator.innerHTML = "";
        state.answers[109] = state.answers[109].filter((i) => i !== idx);
      }

      // Update pending count
      const pendingCount = state.answers[109].length;
