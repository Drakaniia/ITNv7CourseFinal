...
export function initQ30(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[30].includes(idx);
      ...
      const allCorrectSelected = CORRECT[30].every(correctIdx => 
        state.answers[30].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[30].length === CORRECT[30].length) {
        // All correct answers found!
        revealAnswer(30, true);
      }
    });
  });
