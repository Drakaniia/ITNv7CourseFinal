// Initialize Question55
export function initQ54(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-54");
  const submitEl = document.getElementById("submit-54");
  const expEl = document.getElementById("exp-54");
  const verdictEl = document.getElementById("verdict-54");
  const card = document.getElementById("card-54");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[54]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[54].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[54] = state.answers[54].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[54].includes(idx)) {
        state.answers[54].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[54].every(correctIdx => 
        state.answers[54].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[54].length === CORRECT[54].length) {
        // All correct answers found!
        revealAnswer(54, true);
      }
    });
  });
