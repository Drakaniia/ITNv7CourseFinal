// Initialize Question30
export function initQ29(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-29");
  const submitEl = document.getElementById("submit-29");
  const expEl = document.getElementById("exp-29");
  const verdictEl = document.getElementById("verdict-29");
  const card = document.getElementById("card-29");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[29]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[29].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[29] = state.answers[29].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[29].includes(idx)) {
        state.answers[29].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[29].every(correctIdx => 
        state.answers[29].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[29].length === CORRECT[29].length) {
        // All correct answers found!
        revealAnswer(29, true);
      }
    });
  });
