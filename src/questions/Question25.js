// Initialize Question25
export function initQ24(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-24");
  const submitEl = document.getElementById("submit-24");
  const expEl = document.getElementById("exp-24");
  const verdictEl = document.getElementById("verdict-24");
  const card = document.getElementById("card-24");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[24] = state.answers[24].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[24].includes(idx)) {
        state.answers[24].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[24].every(correctIdx => 
        state.answers[24].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[24].length === CORRECT[24].length) {
        // All correct answers found!
        revealAnswer(24, true);
      }
    });
  });
