// Initialize Question63
export function initQ62(ctx) {
  const { state, CORRECT, updateScoreStrip, updateDots } = ctx;
  const optsEl = document.getElementById("opts-62");
  const submitEl = document.getElementById("submit-62");
  const expEl = document.getElementById("exp-62");
  const verdictEl = document.getElementById("verdict-62");
  const card = document.getElementById("card-62");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[62]) return;

      const idx = parseInt(btn.dataset.idx);
      const isCorrectAnswer = CORRECT[62].includes(idx);
      ...
      if (isAlreadySelected) {
        ...
        indicator.innerHTML = "";
        state.answers[62] = state.answers[62].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[62].includes(idx)) {
        state.answers[62].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[62].every(correctIdx => 
        state.answers[62].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[62].length === CORRECT[62].length) {
        // All correct answers found!
        revealAnswer(62, true);
      }
    });
  });
