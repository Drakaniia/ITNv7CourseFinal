...
export function initQ3(ctx) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const isCorrectAnswer = CORRECT[3].includes(idx);
      ...
      if (isAlreadySelected && isCorrectAnswer) {
        ...
        indicator.innerHTML = "";
        state.answers[3] = state.answers[3].filter((i) => i !== idx);
        return;
      }
      ...
      if (!state.answers[3].includes(idx)) {
        state.answers[3].push(idx);
      }

      // Check if all correct answers are selected
      const allCorrectSelected = CORRECT[3].every(correctIdx => 
        state.answers[3].includes(correctIdx)
      );

      if (allCorrectSelected && state.answers[3].length === CORRECT[3].length) {
