// Initialize Question77
export function initQ76({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-76");
  const submitEl = document.getElementById("submit-76");
  const expEl = document.getElementById("exp-76");
  const verdictEl = document.getElementById("verdict-76");
  const card = document.getElementById("card-76");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[76]) return;
      ...
      state.answers[76] = parseInt(btn.dataset.idx);
