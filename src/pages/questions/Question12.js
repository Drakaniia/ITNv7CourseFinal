...
export function initQ11({ state, CORRECT, updateScoreStrip, updateDots }) {
  ...
  const countEl = document.getElementById("pendingCount-11");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const arr = state.answers[11];

      if (btn.classList.contains("selected")) {
        btn.classList.remove("selected");
        btn.querySelector(".opt-indicator").innerHTML = "";
        state.answers[11] = arr.filter((i) => i !== idx);
      } else {
        if (arr.length >= CHOOSE) return; // cap at 3
        btn.classList.add("selected");
        btn.querySelector(".opt-indicator").innerHTML =
          `<svg viewBox="0 0 12 12"><polyline points="2,6 5,9 10,3"/></svg>`;
        state.answers[11] = [...arr, idx];
      }

      const sel = state.answers[11].length;
