// Initialize Question7
export function initQ6({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-6");
  const submitEl = document.getElementById("submit-6");
  const expEl = document.getElementById("exp-6");
  const verdictEl = document.getElementById("verdict-6");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      state.answers[6] = parseInt(btn.dataset.idx);
