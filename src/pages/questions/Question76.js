// Initialize Question76
export function initQ75({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-75");
  const submitEl = document.getElementById("submit-75");
  const expEl = document.getElementById("exp-75");
  const verdictEl = document.getElementById("verdict-75");
  const card = document.getElementById("card-75");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[75]) return;
      ...
      state.answers[75] = parseInt(btn.dataset.idx);
