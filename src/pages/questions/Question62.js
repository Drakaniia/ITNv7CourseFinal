// Initialize Question62
export function initQ61({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-61");
  const submitEl = document.getElementById("submit-61");
  const expEl = document.getElementById("exp-61");
  const verdictEl = document.getElementById("verdict-61");
  const card = document.getElementById("card-61");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[61]) return;
      ...
      state.answers[61] = parseInt(btn.dataset.idx);
