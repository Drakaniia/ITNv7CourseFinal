// Initialize Question81
export function initQ80({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-80");
  const submitEl = document.getElementById("submit-80");
  const expEl = document.getElementById("exp-80");
  const verdictEl = document.getElementById("verdict-80");
  const card = document.getElementById("card-80");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[80]) return;
      ...
      state.answers[80] = parseInt(btn.dataset.idx);
