// Initialize Question66
export function initQ65({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-65");
  const submitEl = document.getElementById("submit-65");
  const expEl = document.getElementById("exp-65");
  const verdictEl = document.getElementById("verdict-65");
  const card = document.getElementById("card-65");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[65]) return;
      ...
      state.answers[65] = parseInt(btn.dataset.idx);
