// Initialize Question65 (Multi-choice: Choose 2)
export function initQ64({ state, CORRECT, updateScoreStrip, updateDots }) {
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const arr = state.answers[64];

      if (btn.classList.contains("selected")) {
        btn.classList.remove("selected");
        btn.querySelector(".opt-indicator").innerHTML = "";
        state.answers[64] = arr.filter((i) => i !== idx);
      } else {
        if (arr.length >= CHOOSE) return;
        btn.classList.add("selected");
        btn.querySelector(".opt-indicator").innerHTML =
          `<svg viewBox="0 0 12 12"><polyline points="2,6 5,9 10,3"/></svg>`;
        state.answers[64] = [...arr, idx];
      }

      const sel = state.answers[64].length;
