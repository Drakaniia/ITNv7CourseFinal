// Initialize Question91
export function initQ90({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-90");
  const submitEl = document.getElementById("submit-90");
  const expEl = document.getElementById("exp-90");
  const verdictEl = document.getElementById("verdict-90");
  const card = document.getElementById("card-90");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[90]) return;
      ...
      state.answers[90] = parseInt(btn.dataset.idx);
