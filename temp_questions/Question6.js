// Initialize Question6
export function initQ5({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-5");
  const submitEl = document.getElementById("submit-5");
  const expEl = document.getElementById("exp-5");
  const verdictEl = document.getElementById("verdict-5");
  ...
  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      state.answers[5] = parseInt(btn.dataset.idx);
