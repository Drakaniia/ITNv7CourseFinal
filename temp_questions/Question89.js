// Initialize Question89
export function initQ88({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-88");
  const submitEl = document.getElementById("submit-88");
  const expEl = document.getElementById("exp-88");
  const verdictEl = document.getElementById("verdict-88");
  const card = document.getElementById("card-88");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[88]) return;
      ...
      state.answers[88] = parseInt(btn.dataset.idx);
