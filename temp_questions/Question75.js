// Initialize Question75
export function initQ74({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-74");
  const submitEl = document.getElementById("submit-74");
  const expEl = document.getElementById("exp-74");
  const verdictEl = document.getElementById("verdict-74");
  const card = document.getElementById("card-74");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[74]) return;
      ...
      state.answers[74] = parseInt(btn.dataset.idx);
