// Initialize Question78
export function initQ77({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-77");
  const submitEl = document.getElementById("submit-77");
  const expEl = document.getElementById("exp-77");
  const verdictEl = document.getElementById("verdict-77");
  const card = document.getElementById("card-77");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[77]) return;
      ...
      state.answers[77] = parseInt(btn.dataset.idx);
