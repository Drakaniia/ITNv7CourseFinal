// Initialize Question51
export function initQ50({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-50");
  const submitEl = document.getElementById("submit-50");
  const expEl = document.getElementById("exp-50");
  const verdictEl = document.getElementById("verdict-50");
  const card = document.getElementById("card-50");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[50]) return;
      ...
      state.answers[50] = parseInt(btn.dataset.idx);
