// Initialize Question92
export function initQ91({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-91");
  const submitEl = document.getElementById("submit-91");
  const expEl = document.getElementById("exp-91");
  const verdictEl = document.getElementById("verdict-91");
  const card = document.getElementById("card-91");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[91]) return;
      ...
      state.answers[91] = parseInt(btn.dataset.idx);
      revealAnswer(91, btn);
    });
  });
