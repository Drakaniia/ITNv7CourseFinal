// Initialize Question96
export function initQ95({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-95");
  const submitEl = document.getElementById("submit-95");
  const expEl = document.getElementById("exp-95");
  const verdictEl = document.getElementById("verdict-95");
  const card = document.getElementById("card-95");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[95]) return;
      ...
      state.answers[95] = parseInt(btn.dataset.idx);
      revealAnswer(95, btn);
    });
  });
