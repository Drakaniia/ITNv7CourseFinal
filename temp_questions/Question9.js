// Initialize Question9
export function initQ8({ state, CORRECT, updateScoreStrip, updateDots }) {
  const optsEl = document.getElementById("opts-8");
  const submitEl = document.getElementById("submit-8");
  const expEl = document.getElementById("exp-8");
  const verdictEl = document.getElementById("verdict-8");
  const card = document.getElementById("card-8");
  const btns = optsEl.querySelectorAll(".option-btn");

  btns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (state.submitted[8]) return;
      ...
      state.answers[8] = parseInt(btn.dataset.idx);
